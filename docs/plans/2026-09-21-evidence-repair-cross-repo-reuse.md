...
| A1 — Qualify and publish Product N boundary | Publisher + reviewer | Exact source/dependency revision, ownership/rights check, ordinary distribution recipe, original-consumer regression | A clean environment imports/invokes the actual retained implementation without private application setup or silently replacing it |
| A2 — Freeze task, evaluator, environment, and budget | Evaluator + coordinator | Public brief/examples, separate held-out evaluator, run manifest and comparison rules | Isolation and instrument negative controls pass before any scored run |
...
Proposed consumer: a **one-shot engineering topic brief**, not a prospecting workflow. Given explicit topic queries, produce a local source-backed JSON/Markdown briefing with stable source IDs, source URLs/text, item/author counts, duplicate suppression by source identity, and explicit per-query failure/partial-result status. No lead ranking, buyer inference, outreach, CRM, scheduler, or new UI. The public brief must define truncation, malformed data, duplicate conflict and error behavior before runs; do not require pagination when the selected scope is explicitly one bounded page.

The task must exercise meaningful existing provider behavior, not merely import a package. If this use case offers no plausible advantage over a small direct integration, record that and choose the one allowed alternative rather than inventing complexity to favor reuse.
...
Acceptance includes full query preservation (including spaces), correct mapping/provenance, duplicates and conflicting duplicates, missing/null fields, empty results, one successful plus one failed query, bounded timeout/error reporting, secret-safe outputs, and declared data-loss/truncation behavior. Deliberately break identity mapping, drop a query, silently swallow a failure, and bypass the retained provider; the relevant evaluator checks must turn red. Consumer acceptance itself remains behavior-based so the control is not penalized for not importing ACA code.
...
| Work | Active engineering ceiling | Model/tool/provider spending cap |
| --- | --- | --- |
| A0–A2 preparation plus evaluation/closeout | 2 hours total, including A1's 60-minute extraction limit | $10 total |
| First control/reuse pair | 30 minutes per worker; 2 workers | $5 per worker; $10 total |
| Optional confirmation pair | Same; at most 2 additional fresh workers | $10 total |
| Optional local effect canary | 45 minutes | $5 total |
| Entire envelope | Under 5 active engineering hours; blocked wall time separate | **$35 maximum** |

No new subscriptions, hosting purchases or infrastructure deployments. Controlled trials make zero paid live-provider requests. At most one optional read-only live smoke request may be authorized separately with a verified cap of $1, included within the $35 rather than added to it. If that cap cannot be verified/enforced, omit it.
