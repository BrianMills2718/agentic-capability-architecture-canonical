...
**ACA-PLAN-002 read-only tranche is complete.** A0 merged in PR #63, A1 in PR #64, and the frozen A2 instrument in PRs #66–67. The scored control/reuse pair is recorded in [A3_A4_RESULT.md](experiments/cross-repo-reuse/A3_A4_RESULT.md). Control reached 33/35 hidden behavioral checks in 295 seconds. Treatment reached 35/35 in 374 seconds, but explicitly rejected the retained Product N asset rather than composing it. The treatment was 26.8% slower and materially more context/list-cost intensive. No second pair is run because the frozen positive criterion required a time improvement in **both** pairs, which the first pair already makes impossible.
...
ACA-PLAN-002 therefore closes with **no demonstrated economic reuse advantage for this asset/task**, plus useful evidence that a fresh worker can reject a plausible but semantically poor fit instead of force-reusing it. A5 effect-safety is deferred because the scored task is read-only and no new effectful-readiness claim is made. The next proof should come from a real product with a genuinely plausible reusable fit or from a recurring failure that survives established engineering mechanisms—not from another ACA platform build or further Twitter refinement.
