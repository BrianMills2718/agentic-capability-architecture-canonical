...
**Current execution route:** [ACA-PLAN-002: evidence repair and cross-repository reuse](plans/2026-09-21-evidence-repair-cross-repo-reuse.md). A0 merged in PR #63. **A1's technical brownfield portability audit is complete**; see [A1_BROWNFIELD_PORTABILITY.md](experiments/cross-repo-reuse/A1_BROWNFIELD_PORTABILITY.md). The pre-existing Product N client is not installable/importable as-is, but a temporary publication-only bundle using byte-identical provider/model files passed the retained multi-query partial-failure behavior. **Next gate: source-owner/redistribution authorization** before any Product N source change or retained artifact. A2–A6 have not started and the scored-run spending cap has not been activated.
...
ACA-PLAN-002 owns sequencing, public versus held-out context, exact revisions, proposed budgets, independent acceptance, marginal versus publication-inclusive cost, and stop rules. Its local effect-safety canary is separate from the read-only comparison. No new runtime, registry service, n8n setup, or paid experiment was performed by A0. Prior-art convergence is complete as a planning gate. The only architectural change made by A0 is the dated ADR-017 scope qualification; ADR-011/013/014/015 are retained. Next step is A1, not another platform build.
