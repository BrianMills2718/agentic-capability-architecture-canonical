...
for app in na_scheduling na_approvals na_notifications acme_rules intake_booking procurement_requests access_requests maintenance_requests service_desk_requests; do
...
for app in na_scheduling na_approvals na_notifications intake_booking; do
  install_if_missing "$SITE_INTAKE" "$app"
done

# Procurement site: shared approvals/notifications + procurement's local extension.
for app in na_approvals na_notifications procurement_requests; do
  install_if_missing "$SITE_PROCUREMENT" "$app"
done

# Access-control site: approvals + notifications + access-specific extension.
for app in na_approvals na_notifications access_requests; do
  install_if_missing "$SITE_ACCESS" "$app"
done

# Maintenance site: Notifications + maintenance-specific extension only.
for app in na_notifications maintenance_requests; do
  install_if_missing "$SITE_MAINTENANCE" "$app"
done

# Service Desk site: Notifications + service-desk-specific extension only.
for app in na_notifications service_desk_requests; do
